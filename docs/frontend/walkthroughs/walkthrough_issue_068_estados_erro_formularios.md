...
O objetivo desta issue foi padronizar e tornar mais robusta a gestão de estados de erro de formulário no frontend do GTH Agents (páginas `NovaMetaPage`, `NovoPDIPage`, `EditarPDIPage` e o componente `AvaliacaoForm`).
...
*   **Páginas**:
    ...
    *   [EditarPDIPage.jsx](../../../frontend/src/pages/EditarPDIPage.jsx) — Implementado o controle isolado de erros no fluxo de edição (`PATCH`) de PDI. Adicionada a limpeza de erros de submissão prévios e invalidação do objeto PDI ao iniciar o carregamento de um novo `pdiId`, evitando o vazamento de estados residuais entre navegações consecutivas sem reload da página.
...
1.  **Isolamento de Estado**: O estado `error` foi desmembrado em `loadError` e `submitError`. O formulário é renderizado após o carregamento inicial, desde que não exista um `loadError`. Qualquer falha na submissão altera apenas o estado `submitError`, exibindo o erro em uma seção dedicada sem forçar a desmontagem da árvore de renderização.
...
5.  **Limpeza no Ciclo de Vida da Navegação**: No `EditarPDIPage.jsx`, o hook `useEffect` limpa explicitamente `submitError` e redefine `pdi` para `null` no início do carregamento de qualquer novo `pdiId`, impedindo que erros e formulários de recursos anteriores permaneçam visíveis enquanto o novo recurso é carregado. O cancelamento por `AbortController` e a checagem `if (controller.signal.aborted) return;` evitam condições de corrida em que requisições canceladas poderiam alterar o estado do componente.
...
*   **ESLint**: Executado `npm run lint` no diretório frontend, finalizado sem erros nem warnings.
...
*   **Navegação entre Edições de PDIs Distintos sem Reload (Cenário 7)**: Confirmado que ao navegar diretamente da edição do PDI A para o PDI B sem reload, os dados anteriores do PDI A são invalidados (evitando a renderização de campos obsoletos) e o `submitError` residual do PDI A é limpo no início do carregamento do novo PDI B.
...
*   **Navegação entre Edições de PDIs Distintos sem Reload (Cenário 7)**:
    ![Erro Transição PDI](../imagens/issue_068_pdi_transicao_limpa.png)
